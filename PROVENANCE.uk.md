...
## Випуск 54.0.2 — акордеони та індикація збереження

Спільне ядро 7.3.2 надає виправлення акордеонів і спільний індикатор Save;
хости зберігають власні правила запису та підтвердження. Випуск узгоджено зі
Standard Notes AIC 46.0.2 та браузером 0.9.2. `CORE_SNAPSHOT.json` має фіксувати
канонічний commit і точні хеші файлів перед перевіркою випуску. Двомовний
`RELEASE_INSTALL.md` однаковий в обох репозиторіях.

## Випуск 53.0.1 — генерація пароля на вузьких екранах

Спільне ядро 7.3.1 повертає генерацію для порожніх парольних полів на будь-якій
ширині та переносить параметри у вузьких панелях. Головний редактор і Linked Note
мають те саме виправлення, що Standard Notes AIC 45.0.1 і браузер 0.9.1.
`CORE_SNAPSHOT.json` фіксує канонічну ревізію та хеші; спільні контролі зберігають
захист наявних значень і нотаток у режимі читання.

## Випуск 52.1.0 — читабельні записи та сортування після редагування

Спільне ядро 7.3.0 надає перенесення полів природної ширини, копіювання пароля
кнопкою із замком і стабільне сортування рядків при виході з джерела. Головний
редактор і Linked Note використовують ту саму копію ядра. `CORE_SNAPSHOT.json`
містить канонічний commit і точні хеші файлів. Випуск узгоджений зі Standard
Notes AIC 44.1.0 та експериментальним браузером 0.9.0. Двомовний
`RELEASE_INSTALL.md` спільний для обох сторінок GitHub і входить до VSIX.
