...
Download `aic-notes-22.1.1.vsix` and `aic-notes-22.1.1.vsix.sha256` from the
[v22.1.1 release](https://github.com/ldzyha/aic-notes/releases/tag/v22.1.1). The VSIX is universal:
use the same file on Windows, Linux, macOS, and code-server.

Windows PowerShell:

```powershell
(Get-FileHash .\aic-notes-22.1.1.vsix -Algorithm SHA256).Hash.ToLower()
Get-Content .\aic-notes-22.1.1.vsix.sha256
code --install-extension .\aic-notes-22.1.1.vsix --force
```

Linux, macOS, or code-server:

```sh
sha256sum -c aic-notes-22.1.1.vsix.sha256
code --install-extension ./aic-notes-22.1.1.vsix --force
# or: code-server --install-extension ./aic-notes-22.1.1.vsix --force
```
...
`22.1.1` is sequence 22 with one feature outcome and one fixed-bug outcome.
