...
Download `aic-notes-16.2.0-linux-x64.vsix` and its `.sha256` file from the
[v16.2.0 release](https://github.com/ldzyha/aic-notes/releases/tag/v16.2.0), then verify and install:

```sh
sha256sum -c aic-notes-16.2.0-linux-x64.vsix.sha256
code-server --install-extension aic-notes-16.2.0-linux-x64.vsix --force
```
...
```powershell
code --install-extension .\aic-notes-16.2.0-win32-x64.vsix --force
```
...
release-local fixed-bug outcomes. `16.2.0` is sequence 16 with two feature outcomes and no
