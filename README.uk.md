...
Випуск **53.0.1** на спільному ядрі **7.3.1** повертає генерацію пароля у вузькі
панелі головного редактора та Linked Note. Він узгоджений зі Standard Notes AIC
**45.0.1** та експериментальним розширенням браузера **0.9.1**.
