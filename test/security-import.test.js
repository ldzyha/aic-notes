...
import {
  ...
  SECURITY_FENCE_INFO,
} from "../vendor/aic-editor-core/field-syntax.js";
...
test("vendored Authenticator conversion preserves independent masked records", () => {
  ...
  const blocks = result.markdown
    .split("\n\n")
    .map((block) =>
      parseSecurityBlock(
        block.slice(("```" + SECURITY_FENCE_INFO + "\n").length, -3),
        SECURITY_FIELD_OPTIONS,
      ),
    );
  ...
  assert.deepEqual(
    blocks.flatMap((block) =>
      block.model.sections.map((section) =>
        section.fields.find((field) => field.label === "TOTP"),
      ),
    ),
    records.map((record) => ({
      label: "TOTP",
      parts: [{ value: record.secret, kind: "totp" }],
    })),
  );
  assert.deepEqual(
    convertAuthenticatorJson(
      JSON.stringify([records[0], { ...records[1], secret: false }]),
    ),
...
test("vendored import splits overflow into valid v3 blocks without requiring titles", () => {
  ...
  assert.deepEqual(
    sections.map((section) => section.fields[0].parts[0].value),
...
test("both webview surfaces mount the shared contextual importer and CSS", async () => {
  ...
  assert.match(
    main,
    /import \{ makeSecurityImportExtension \} from "\.\.\/\.\.\/vendor\/aic-editor-core\/security-import-extension\.js"/u,
  );
  assert.match(
    main,
    /makeSecurityImportExtension\(\{ onSave: saveCurrentDraft \}\)/u,
  );
