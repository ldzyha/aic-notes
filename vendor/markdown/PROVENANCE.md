...
| `handlers/frontmatter.js` | removed     | retired Properties table/drag handler; frontmatter now renders through the shared `vendor/aic-editor-core/security-block.js` Properties card                                                                                                             |
...
| `mermaid.js`              | adapted     | VS Code widget/host routing delegates rendering to the shared strict Mermaid runtime and preview zoom/scroll to the shared viewport. Markdown source remains directly editable with a live preview. The visual builder and console preview are not used. |
